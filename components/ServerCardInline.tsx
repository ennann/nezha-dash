...
export default function ServerCardInline({
  ...
  return online ? (
    <Link onClick={saveSession} href={`/server/${id}`} prefetch={true}>
      <Card
        className={cn(
          "flex items-center lg:flex-row justify-start gap-3 p-3 md:px-5 cursor-pointer hover:bg-accent/50 transition-colors min-w-[980px] w-full",
        )}
      >
        <section className={cn("flex w-56 flex-col gap-1")}>
          <div
    ...
    <Card
      className={cn(
        "flex items-center justify-start gap-3 p-3 md:px-5 min-h-[61px] min-w-[980px] flex-row",
      )}
    >
      <section className={cn("flex w-56 flex-col gap-1")}>
        <div
